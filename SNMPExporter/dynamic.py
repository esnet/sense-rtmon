...
if len(sys.argv) <= 1 or str(sys.argv[1]) == "-h" or str(sys.argv[1]) == "-H":
    ...
    try:
       ...
       # Read in the file
        ...
        subprocess.run("sudo mv generator.yml ${GOPATH-$HOME/go}/src/github.com/prometheus/snmp_exporter/generator", shell=True)
        subprocess.run("go build", shell=True, cwd='go/src/github.com/prometheus/snmp_exporter/generator')
        subprocess.run("make mibs", shell=True, cwd='go/src/github.com/prometheus/snmp_exporter/generator')
        print("Generating dynamic SNMP config file...")
        subprocess.run("./generator generate", shell=True, cwd='go/src/github.com/prometheus/snmp_exporter/generator')
        print("Initializing docker container with custom SNMP exporter...")
        subprocess.run("sudo docker run -d --name snmp-exporter -p 9116:9116     -v $PWD/snmp.yml:/etc/snmp_exporter/snmp.yml prom/snmp-exporter", shell=True, cwd='go/src/github.com/prometheus/snmp_exporter/generator')
