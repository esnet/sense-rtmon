...
class RTMonWorker(
    ...
    def _updateState(self, filename, fout):
        ...
        with open(
            f'{self.config.get("workdir", "/srv")}/{filename}', "w", encoding="utf-8"
        ) as fd:
    ...
    def _checkSenseOState(self, fout):
        ...
        if out["state"] not in self.goodStates:
            self.logger.info(
                f'Instance {instanceuuid} is in good state: {out["state"]}'
            )
            return False
        return True
    ...
    def renew_exe(self, filename, fout):
        ...
        try:
            ...
            self.s_setTaskState(
                fout.get("taskinfo", {}).get("uuid", ""), "REJECTED", {"error": msg}
            )
            return
        ...
        self.s_finishTask(
            ...
            {
                "callbackURL": self.g_getDashboardURL(
                    template["dashboard"]["title"], self._getFolderName()
                )
            },
        )
            ...
            if present:
                ...
                if fout["taskinfo"]["status"] != "FINISHED":
                    self.s_finishTask(
                        ...
                        {
                            "callbackURL": self.g_getDashboardURL(
                                dashbVals["title"], self._getFolderName()
                            )
                        },
                    )
                    fout["taskinfo"]["status"] = "FINISHED"
                ...
                if self.config["template_tag"] in dashbVals["tags"]:
                    ...
                    if not self._checkSenseOState(fout):
                        self.logger.info(
                            "SENSE-O Task State not in a final state. Will delete the dashboard"
                        )
                        fout["state"] = "delete"
                        self._updateState(filename, fout)
                    return
    ...
    def failed_exe(self, filename, fout):
        ...
        fout.setdefault("retries", 0)
        self.logger.info(
            f'Will mark it as delete after 10 cycles. Current: {fout["retries"]}'
        )
        fout["retries"] += 1
    ...
    def _taskCancel(self, task, filename):
        ...
        if not os.path.exists(fullpathfilename):
            self.logger.info(
                f"File {fullpathfilename} not found on the server. RTMon has no knowledge about this monitoring instance"
            )
    ...
    def _taskAccept(self, task, filename):
        ...
        if out["state"] in self.goodStates:
            self.auth_instances[os.environ["SENSE_AUTH_OVERRIDE_NAME"]].append(
                out["referenceUUID"]
            )
            ...
            msg = (
                f'Instance not in correct state: {out["referenceUUID"]}, {out["state"]}'
            )
    ...
    def _getAllTasks(self):
        ...
        for task in newtasks:
            ...
            if not instanceuuid:
                msg = (
                    f"Instance UUID not found in task provided by SENSE-O. Task: {task}"
                )
                self.logger.error(msg)
                self.s_setTaskState(task["uuid"], "REJECTED", {"error": msg})
                continue
            filename = (
                f'rtmon-debug-{os.environ["SENSE_AUTH_OVERRIDE_NAME"]}-{instanceuuid}'
            )
            ...
            if task.get("config", {}).get("register", None) is True:
                ...
                msg = (
                    f"Register flag not found in task provided by SENSE-O. Task: {task}"
                )
                self.logger.error(msg)
                self.s_setTaskState(task["uuid"], "REJECTED", {"error": msg})
    ...
    def main(self):
        ...
        for state in ["submitted", "delete", "running", "failed", "renew"]:
            self.logger.info(
                "State: %s, Files: %s", state, len(stateInfo.get(state, {}))
            )
    ...
    def _startwork(self):
        ...
        for key, val in self.config.get("sense_endpoints", {}).items():
            try:
                ...
                self.logger.error("SENSEOFailure: %s", ex)
                self.logger.error(
                    "Failed to load SENSE-O data. Will not check any local files"
                )
                raise ex
        startTime = int(time.time())
