...
# holders for host interface index
host_if_index = []
...
if_index1 = cloud_functions.index_finder(pushgateway_metrics,str(data["hostA"]["port"]["ifName_A"]))
if_index2 = cloud_functions.index_finder(pushgateway_metrics,str(data["hostB"]["port"]["ifName_A"]))
for i in range(switch_num):
    letter = chr(ord('A')+i) # A B C D ... 
