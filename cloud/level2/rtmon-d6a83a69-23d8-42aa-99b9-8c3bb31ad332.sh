#!/bin/bash 
        ...
        # check_arp_on
        if curl http://172.31.72.189:9091/metrics | grep '.*arp_state.*' | grep '.*flow="rtmon-d6a83a69-23d8-42aa-99b9-8c3bb31ad332".*' | grep '.*instance="T2_US_UCSD:k8s-igrok-02.calit2.optiputer.net".*'; then
        ...
        if curl http://172.31.72.189:9091/metrics | grep '.*arp_state.*' | grep '.*flow="rtmon-d6a83a69-23d8-42aa-99b9-8c3bb31ad332".*' | grep '.*instance="T2_US_UCSD:k8s-igrok-02.calit2.optiputer.net".*' | grep '.*IPaddress="10.251.85.190".*'; then
    ...
    # check_snmp_on
    if curl http://172.31.72.189:9091/metrics | grep '.*ifHCInOctets.*' | grep '.*instance="T2_US_UCSD:s1".*' | grep '.*flow="rtmon-d6a83a69-23d8-42aa-99b9-8c3bb31ad332".*'; then
    ...
    # check_snmp_on
    if curl http://172.31.72.189:9091/metrics | grep '.*ifHCInOctets.*' | grep '.*instance="calit2.optiputer.net:2020:prism-core".*' | grep '.*flow="rtmon-d6a83a69-23d8-42aa-99b9-8c3bb31ad332".*'; then
