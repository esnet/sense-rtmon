...
if curl ${pushgateway}:9091/metrics | grep "dot1.*instance=\"${host1}\".*job=\"snmp-exporter\".*"; then
    echo "host1_snmp_on_${vlan_num}{host=\"${host1}\"} 1";
else 
    echo "host1_snmp_on_${vlan_num}{host=\"${host1}\"} 0";
fi
if curl ${pushgateway}:9091/metrics | grep "dot1.*instance=\"${host2}\".*job=\"snmp-exporter\".*"; then
    echo "host2_snmp_on_${vlan_num}{host=\"${host2}\"} 1";
else 
    echo "host2_snmp_on_${vlan_num}{host=\"${host2}\"} 0";
fi
...
if curl ${pushgateway}:9091/metrics | grep ".*dot1dTpFdbAddress=${host1_mac^^}.*vlan=\"${vlan_num}\".*"; then
    echo "switch_host1_mac_${vlan_num}{host=\"${switch_ip1}\"} 1";
    echo "host1_mac{mac=\"${host1_mac_no_quote}\"} 1"
else 
    echo "switch_host1_mac_${vlan_num}{host=\"${switch_ip1}\"} 0";
fi
if curl ${pushgateway}:9091/metrics | grep ".*dot1dTpFdbAddress=${host2_mac^^}.*vlan=\"${vlan_num}\".*"; then
    echo "switch_host2_mac_${vlan_num}{host=\"${switch_ip1}\"} 1";
    echo "host2_mac{mac=\"${host2_mac_no_quote}\"} 1"
else 
    echo "switch_host2_mac_${vlan_num}{host=\"${switch_ip1}\"} 0";
fi
